...
function cheatYield(battle) {
    battle.yieldCooldown = 0;
    if (battle.headless) return Promise.resolve();
...
class Weapon {
    ...
    static drawState(ctx, state, ball) {
        if (!state.sprite) return;
        Weapon.drawWeapon(
            ctx, ball.x, ball.y,
            state.theta, state.sprite, state.scale,
            ball.radius + state.offset, state.spriteShift, state.rotation
        );
    }
...
class Ball extends CircleBody {
    ...
    checkSlamDamage(other = null) {
        if (this.slamTimer > 0 && !(this.slamSource == other && this.slamTimer == (this.battle.mode == DUEL ? duelSlam : FFASlam))) {
            ...
            let dmg;
            if (/*other instanceof SnakeSegment && other.owner == this || other == this.slamSource && this instanceof SnakeBall*/this.segments && this.segments.length > 0) {
                dmg = 1;
            }
            else if (this.battle.mode == DUEL) {
                dmg = this.slamTimer > 14 ? 3 :
                    this.slamTimer > 9 ? 2 :
                        1;
            }
            else {
    ...
    onCollision(b) {
        ...
        if (this.isStunned() && !(b instanceof Ball && b.isStunned()) && !(b instanceof Bullet)) {
    ...
    damage(dmg, source = null) {
        ...
        super.damage(dmg);
        // Always recorded (even headless/forked battles) so Clover's ghost lookahead
        // can tell real damage events apart from incidental HP drops (e.g. VampireBall's
        // passive bleed), which don't warrant a flash.
        this.battle._flashEvents?.push(this.id);
        ...
        if (source && this.segments) {
            for (let s of this.segments) {
                // const len0 = s.deferredHits.length;
                s.deferredHits = s.deferredHits.filter((x) => source != x.source);
                // if (len0 != s.deferredHits.length) console.log(t, "cleared");
            }
        }
    ...
    // Default: no extra per-type stats to snapshot for getInfoEl(). Overridden
    // by subclasses whose getInfoEl() needs specific instance fields, so the
    // UI (which reads snapshots from the buffered frame queue, not live balls)
    // never displays stats from ticks ahead of what's rendered.
    getScalingStats() {
        return {};
    }
    ...
    copyRenderState() {
        return {
            ...
            hpText: Ball.hpText(this.hp),
            stunned: this.isStunned(),
            weapons: this.weapons.map(w => w.copyRenderState()),
            drawState: Ball.drawState,
            getInfoEl: this.getInfoEl ? this.getInfoEl.bind(this, this.getScalingStats()) : null,
        };
    }

    static drawState(ctx, state, frameCtx) {
        const color = Ball.flashColor(state.color, frameCtx?.flashState?.get(state.id));
        ...
        for (const w of state.weapons) Weapon.drawState(ctx, w, state);
        Ball.drawBall(ctx, x, y, state.radius, color, state.hpText, state.x, state.y);
    }

    showDmg(dmg, comboGroup = 0, isHeal = false) {
        if (this.battle.headless) return;

        const hpBeforeCeil = this.hp == Infinity ? Infinity : Math.ceil(this.hp + (isHeal ? -dmg : dmg));
        this.battle._hitEvents?.push({
            ballId: this.id,
            isSnakeSegment: this instanceof SnakeSegment,
            dmg,
            color: this.color,
            comboGroup,
            isHeal,
            hpBeforeCeil,
            hpAfterCeil: this.hp == Infinity ? Infinity : Math.ceil(this.hp),
            // Captured now (while the ball is still alive) as the indicator's spawn
            // position, since processDeaths() removes dead balls from battle.bodies
            // before render() snapshots the frame -- so a killing blow's indicator
            // would otherwise have no live body to read a position from.
            x: this.x,
            y: this.y,
            radius: this.radius,
        });
    ...
    onUpdate(dt) {
        ...
        if (stillStunned) {
            ...
            if (this.isStunned()) {
                // Waking up: clear any slow-time effects so the ball doesn't
                // wake up already slowed.
                this.slowTime = 0;
                this.slowFactor = 1;
            }
            this.clearStun();
...
class Wall {
    ...
    resolve(b) {
        ...
        if (!(b instanceof Turret) && Math.abs(wallVel) > EPS) {
            ...
            else if (b.battle.t > 0) console.log(`[t=${b.battle.t}] removed ${-addedKE} KE (boost=${boost})`);
        }
    }
}
...
function plusArenainRectBoundsFromWalls(x, y, r, walls, corners) {
    for (let i = 0; i < walls.length; i++) {
        ...
        if (isHoleWall) {
            const depth = Math.abs(walls[i ^ 1].pos - w.pos);
...
// nx, ny should point from the pinned body toward b.
function reflectOffPinnedBody(b, nx, ny, wallVx, wallVy) {
    ...
    if (dot < 0) {
        ...
        const wallVx2 = b.vx - 2 * dot * nx * speedTaper;
        const wallVy2 = b.vy - 2 * dot * ny * speedTaper;
...
function bounceOffWeaponFace(weapon, wielder, b, bounceThresh = 1) {
    ...
    // if (wielder instanceof MagnetBall || b instanceof MagnetBall) {
    //     const magnet = wielder instanceof MagnetBall ? wielder : b;
    //     console.log(`[t=${weapon.ball.battle.t}] bounceOffWeaponFace: wielder=${wielder.constructor.name}#${wielder.id} b=${b.constructor.name}#${b.id} theta=${theta.toFixed(3)} n=(${nx.toFixed(3)},${ny.toFixed(3)}) relV=(${relVx.toFixed(3)},${relVy.toFixed(3)}) velAlongNormal=${velAlongNormal.toFixed(4)} bounceThresh=${bounceThresh} bounced=${bounced} magnet.pos=(${magnet.x.toFixed(1)},${magnet.y.toFixed(1)})`);
    // }
...
function predictedWeaponDist(w, b, stationary = false, t = 10) {
    w.theta += w.angVel * t;
    let min = Infinity;
    for (const seg of w.getHitSegments()) {
        if (stationary) min = Math.min(min, distToRect(b.x, b.y, seg.x1, seg.y1, seg.x2, seg.y2, seg.r));
        else min = Math.min(min, distToRect(b.x + t * b.vx, b.y + t * b.vy, seg.x1 + t * w.ball.vx, seg.y1 + t * w.ball.vy, seg.x2 + t * w.ball.vx, seg.y2 + t * w.ball.vy, seg.r));
    }
    w.theta -= w.angVel * t;
    return min;
}

function iframeKeyFor(target) {
    return target instanceof SnakeSegment ? "snake" + target.owner.id : target.id;
}
...
class BallBattle {
    constructor(balls, seed, gravity = 0.1, mode = DUEL, headless = typeof document === "undefined") {
        ...
        this.bodies = [];
        this.dots = [];
        this.particles = [];      // consumer-owned death particles (see renderLoop())
        this.dmgIndicators = [];  // consumer-owned damage indicators (see renderLoop())
        this.frameQueue = [];     // production -> consumer snapshot queue (see render())
        this._hitEvents = [];
        this._flashEvents = [];
        this._deathEvents = [];
    ...
    updateTimeScale() {
        const isDupeDuel = this.balls[0] instanceof DuplicatorBall || this.balls[1] instanceof DuplicatorBall;
        const dupeVsVamp = isDupeDuel && (this.balls[0] instanceof VampireBall || this.balls[1] instanceof VampireBall);
        ...
        if (this.mode == DUEL) {
            ...
            for (const b of this.balls) {
                if (!b.isStunned()) {
                    if (b.owner instanceof GrimoireBall) {
                        ...
                        else if (b instanceof VampireBall) count += 0.5;
                        else count++;
                    }
                    else {
            ...
            const dupeVsMirror = isDupeDuel && this.balls[0] instanceof MirrorBall || this.balls[1] instanceof MirrorBall;
        ...
        if (this.baseTimeScale == null || (this.baseTimeScale > 1 && this.targetTimeScale < this.baseTimeScale)) this.baseTimeScale = this.targetTimeScale;
        ...
        if (this.mode == DUEL) {
            ...
            this.timeScale *= this._ghostSlowFactor ?? 1;
        ...
        for (const b of this.balls) {
            for (let wi = 0; wi < b.dmgWeapons.length; wi++) {
                ...
                for (const id of Object.keys(w.iFrames)) {
                    ...
                    if ((w.DoT && !id.startsWith("snake")) || !this.hitThisFrame.has(w.ball.id + "-" + wi + "-" + id)) {
                        w.iFrames[id]--;
    ...
    processDeaths() {
        ...
        dead.forEach((b) => {
            ...
            if (!this.headless) {
                ...
                this._deathEvents?.push({ x: b.x, y: b.y, color: b.color, count });
            }
    ...
    cloverGhostTimeScaleAt() {
        ...
        const tickPos = Math.max(0, this.t - ghosts.startTick);
        return ghosts.timeScales[Math.min(tickPos, ghosts.timeScales.length - 1)];
    }
    ...
    updateCloverGhosts() {
        ...
        // Ghost (the un-cheated "what would have happened") renders at full
        // opacity while destiny is still being rewritten, then fades out as the
        // real timeline fades in (see realOpacityForPath below) to take its place.
        const baseOpacity = 1;
        const opacityForPath = (pathIdx) => {
            ...
            const opacity = ticksSinceFade <= 0 ? baseOpacity : ticksSinceFade >= fadeOutDur ? 0 : baseOpacity * (1 - ticksSinceFade / fadeOutDur);
            return opacity;
        };
        // Real timeline's opacity: starts hidden while the ghost shows the
        // un-cheated path, then fades in exactly as the ghost fades out.
        ...
        for (let pathIdx = 0; pathIdx < ghosts.paths.length; pathIdx++) {
            ...
            if (i0 >= path.length) continue; // finished
            if (opacityForPath(pathIdx) <= EPS) continue; // fully faded
            keep.push(pathIdx);
        }
        if (keep.length < ghosts.paths.length) {
            if (keep.length === 0) {
                this.cloverGhosts = null;
                this._ghostSlowFactor = 1;
                return null;
            }
            const remapCache = (arr) => arr && keep.map(i => arr[i]);
            ghosts.paths = keep.map(i => ghosts.paths[i]);
            ghosts.rewards = remapCache(ghosts.rewards);
            ghosts.timeScales = remapCache(ghosts.timeScales);
            ghosts._fadeStartTick = remapCache(ghosts._fadeStartTick);
        }

        const perBody = new Map(); // id -> [{x, y, ownerId, color, opacity}, ...]
        for (let pathIdx = 0; pathIdx < ghosts.paths.length; pathIdx++) {
            const path = ghosts.paths[pathIdx];
            if (i0 >= path.length) continue;
            const opacity = opacityForPath(pathIdx);
            for (const body of path[i0]) {
                if (body.hp <= 0) continue;
                let list = perBody.get(body.id);
                if (!list) perBody.set(body.id, list = []);
                list.push({ x: body.x, y: body.y, ownerId: body.ownerId, color: body.color, opacity });
            }
        }
        for (const b of this.bodies) {
            if (b.hp <= 0) continue;
            const list = perBody.get(b.id);
            if (list) list.push({ x: b.x, y: b.y, ownerId: b.owner ? b.owner.id : null, color: b.color });
        }
        ...
        for (let pathIdx = 0; pathIdx < ghosts.paths.length; pathIdx++) {
            const path = ghosts.paths[pathIdx];
            if (i0 >= path.length) continue;
            realOpacity = Math.max(realOpacity, realOpacityForPath(pathIdx));
            const opacity = opacityForPath(pathIdx);
            if (opacity <= EPS) continue;
            snapshot.push({ pathIdx, path, i0, opacity });
        }
        snapshot.realOpacity = realOpacity;
        return snapshot;
    }
        ...
        for (const { pathIdx, path, i0, opacity } of snapshot) {
            ...
            for (const body of cur) {
                ...
                if (!body.isBall) {
                    ...
                    const state = lerp ? {
                        ...body,
                        x: body.x + (nb.x - body.x) * lerp,
                        y: body.y + (nb.y - body.y) * lerp,
                        theta: body.theta !== undefined && nb.theta !== undefined ? body.theta + (nb.theta - body.theta) * lerp : body.theta,
                    } : body;
                    body.drawState(ctx, state);
                    continue;
                }
                ...
                for (let wi = 0; wi < body.weapons.length; wi++) {
                    ...
                    const theta = nw ? w.theta + (nw.theta - w.theta) * lerp : w.theta;
                    Weapon.drawWeapon(ctx, x, y, theta, w.sprite, w.scale, body.radius + w.offset, w.spriteShift, w.rotation);
                }

                const color = Ball.flashColor(body.color, flashTimes.get(flashKey), currentTime);
                Ball.drawBall(ctx, x, y, body.radius, color, Ball.hpText(hp));
            }
        }
        ctx.globalAlpha = 1;
    }
    ...
    render() {
        ...
        const bodies = [...this.bodies]
            ...
            .map(b => b.copyRenderState());

        const dots = this.dots.map(d => d.copyRenderState());

        let debugSegments = null;
        if (this.debug) {
            debugSegments = [];
            for (const b of this.balls) {
                for (const w of b.weapons) {
                    if (w.range) {
                        for (const seg of w.getHitSegments()) debugSegments.push(seg);
                    }
                }
            }
        }
        ...
        this.frameQueue.push({
            zoom: this.zoom ?? 1,
            drawArena: this.drawArena,
            timeScale: this.timeScale,
            bodies,
            dots,
            debugSegments,
            cloverGhosts: this._cloverGhostFrame ?? null,
    ...
    renderLoop(dt) {
        this.flashState ??= new Map(); // ballId -> expiry timestamp (performance.now() + flashDur)
        this.frameAccum = dt;
        ...
        const loop = (currentTime) => {
            ...
            if (drainedFrame) this._lastFrame = drainedFrame;

            if (this._lastFrame) {
                const frame = this._lastFrame;
                const realDt = this._lastRenderTime != null ? currentTime - this._lastRenderTime : 16.6667;
                this._lastRenderTime = currentTime;

                const frac = Math.max(0, Math.min(1, this.frameAccum / dt));
                const nextFrame = this.frameQueue[0];
                let drawBodies = frame.bodies;
                if (frac > 0 && nextFrame) {
                    const nextById = new Map();
                    for (const nb of nextFrame.bodies) {
                        if (nb.id !== undefined) nextById.set(nb.id, nb);
                    }
                    drawBodies = frame.bodies.map(b => {
                        if (b.id === undefined) return b; // SoulDot: not interpolated (no id)
                        ...
                        if (b.theta !== undefined && nb.theta !== undefined) {
                            interp.theta = b.theta + (nb.theta - b.theta) * frac;
                        }
                        if (b.weapons?.length) {
                            interp.weapons = b.weapons.map((w, wi) => {
                                const nw = nb.weapons?.[wi];
                                if (!nw || !w.interpolate) return w;
                                return { ...w, theta: w.theta + (nw.theta - w.theta) * frac };
                            });
                        }
                        return interp;
                    });
                ...
                this.ctx.restore();
                this.canvas.style.transform = `scale(${frame.zoom})`;

                frame.drawArena(this.ctx);

                if (frame.debugSegments) {
                    this.ctx.globalAlpha = 0.5;
                    for (const seg of frame.debugSegments) {
                        this.ctx.beginPath();
                ...
                for (const p of this.particles) DeathParticle.drawState(this.ctx, p.copyRenderState());
                ...
                for (const [ballId, expiry] of this.flashState) {
                    if (expiry <= currentTime) this.flashState.delete(ballId);
                }
                ...
                if (drainedHitEvents) {
                    for (const ev of drainedHitEvents) {
                        ...
                        if (existing) {
                            existing.setDmg(existing.dmg + ev.dmg, ev.hpAfterCeil);
                            existing.reset(ev.radius);
                        } else {
                            const ind = new DamageIndicator(ev.ballId, ev.dmg, ev.color, ev.comboGroup, ev.isHeal, ev.hpBeforeCeil, ev.hpAfterCeil);
                            if (ev.isSnakeSegment || ind.displayedDmg > 0) {
                                ind.reset(ev.radius);
                                ind.x = ev.x;
                                ind.y = ev.y - ind.yOffset;
                                this.dmgIndicators.push(ind);
                            }
                        }
                    }
                }
                for (let i = this.dmgIndicators.length - 1; i >= 0; i--) {
                    ...
                    const pos = bodyById.get(d.ballId);
                    d.update(realDt, pos);
            ...
            const pacingTimeScale = this._lastFrame ? this._lastFrame.timeScale : this.timeScale;
            const realFrameDt = this._lastFrameTime != null ? currentTime - this._lastFrameTime : 0;
            this._lastFrameTime = currentTime;
            this.frameAccum += realFrameDt * pacingTimeScale;
            this.frameAccum = Math.min(this.frameAccum, dt * 100);
    ...
    async update() {
        ...
        for (const b of this.balls) {
            ...
            for (let pass = 0; pass < snakeLinkPasses; pass++) {
                for (let i = 0; i < chain.length - 1; i++) {
                    ...
                    const restDist = a.radius + c.radius - (b.giga ? 7.5 : snakeSegOverlap);
            ...
            if (ke > EPS && ke > budget) {
                ...
                const decay = b.extraEnergy * snakeExtraEnergyDecay * b.getTimeScale();
        ...
        if (this._pendingCloverCheats) {
            ...
            for (const { clover, bounced, reflected } of pending) {
                if (clover.hp <= 0) continue;
                if (reflected == null) {
                    await clover.cheat(bounced);
                }
                else {
                    // Workaround so Mirror cheats once when it reflects Clover
                    clover.foresight = reflected.foresight;
                    clover.cheatCooldown = 0;
                    clover.cheat = CloverBall.prototype.cheat;
                    clover.scoreCandidate = CloverBall.prototype.scoreCandidate;

                    await clover.cheat(bounced);
                    // reflected.cheatCooldown = clover.cheatCooldown;
                }
            }
        }
    }
    ...
    async physicsLoop(dt, maxQueueLen = 100) {
        while (!this.stopped) {
            ...
            if (this.yieldCooldown >= this.getYieldTicks()) await cheatYield(this);
        }
    }

    getYieldTicks() {
        return (this.mode == DUEL ? 500 : 250) / this.colliderCount;
    }
    ...
    async run(dt) {
        ...
        if (!this.headless) this.renderLoop(dt);
        await this.physicsLoop(dt);
    }
    ...
    evilFork() {
        ...
        const rec = (_new, obj) => {
            const loop = (key) => {
                if (obj == this && ["ctx", "dmgIndicators", "particles", "frameQueue", "flashState", "cloverGhosts", "_cloverGhostFrame", "_cloverGhostFlashTimes"].indexOf(key) != -1) {
                    return;
                }
...
let feedMatrix = {};
function recordFeed(from, to) {
    if (to.owner || to instanceof MirrorBall || from.battle.evil) return;
    from = from.getRootOwner();

    const getKey = (ball) => {
        let name = ball.constructor.name;
        if (name.endsWith("Ball")) name = name.substring(0, name.length - 4);
        return name;
    }

    const fromKey = getKey(from);
    if (feedMatrix[fromKey] == null) feedMatrix[fromKey] = {};

    const toKey = getKey(to);
    if (feedMatrix[fromKey][toKey] == null) feedMatrix[fromKey][toKey] = 0;

    feedMatrix[fromKey][toKey]++;
}
...
class DuplicatorBall extends Ball {
    ...
    handleCollision(b, reflector) {
        ...
        // const theta = Math.atan2(this.y - b.y, this.x - b.x);
    ...
    getScalingStats() {
        return {
            population: this.battle.teamCount ? this.battle.teamCount[this.team] : 1,
        }
    }
...
class DaggerBall extends Ball {
    ...
    getScalingStats() {
        return { angVel: this.weapons[0].angVel };
    }

    getInfoEl(stats) {
        return this.propsToList({
            "Spin Boost": { text: Math.round((Math.abs(stats.angVel) - this.baseSpin) * 100 / this.baseSpin) + "%", grad: { from: 0, to: 750 } },
        });
    }
...
class SwordBall extends Ball {
    ...
    getScalingStats() {
        return { dmg: this.weapons[0].dmg };
    }

    getInfoEl(stats) {
        return this.propsToList({
            "Damage": { text: stats.dmg, grad: { from: 1, to: 15 } },
        });
    }
...
class LanceBall extends Ball {
    constructor(x, y, vx, vy, hp = 100, radius = 25, color = "#dfbf9f", mass = radius * radius) {
        ...
        lance.ballColFns.push((target, reflector) => {
            ...
            const snakeIFrames = this.battle.mode == DUEL ? 1 : 3;
    ...
    getScalingStats() {
        return { boosts: this.boosts };
    }

    getInfoEl(stats) {
        return this.propsToList({
            "Speed Boost": { text: Math.round(stats.boosts * 100 * boostPct) + "%", grad: { from: 0, to: 150 } },
...
const bulletRadius = 5, maxVolley = 110, reloadTime = 61;
class MachineGunBall extends Ball {
    ...
    getScalingStats() {
        return { total: this.damagePerRound + this.pendingDamage };
    }

    getInfoEl(stats) {
        return this.propsToList({
            "Bullets": { text: stats.total, grad: { from: 10, to: 110 } },
        });
    }
}
...
class Bullet extends CircleBody {
    ...
    copyRenderState() {
        ...
        return {
            id: this.id,
            x: this._renderX ?? this.x,
            y: this._renderY ?? this.y,
            radius: this.radius,
            color: this.color,
            alpha: Math.min(1, this.lifetime / fadeStart),
            drawState: Bullet.drawState,
        };
    }

    static drawState(ctx, state) {
        ctx.globalAlpha = state.alpha;
        ctx.strokeStyle = "#333";
        ctx.lineWidth = 1;
        ctx.fillStyle = state.color;
        ctx.beginPath();
        ctx.arc(state.x, state.y, state.radius - ctx.lineWidth / 2, 0, Math.PI * 2);
        ctx.stroke();
        ctx.fill();
        ctx.globalAlpha = 1;
    }
...
class MGBullet extends Bullet {
    ...
    handleCollision(b) {
        ...
        if (b instanceof Ball /*&& !(b instanceof SnakeSegment)*/) {
            const hc = this.resolveHitCredit(b);
            if (!b.owner && !(b instanceof DuplicatorBall)) {
                let h = [b];
            ...
            if (hc instanceof MirrorBall || hc.team == this.owner.team) {
                const targetId = b instanceof SnakeSegment ? b.owner.id : b.id;
                if (this.scalingDedupe.indexOf(targetId) == -1) {
                    this.owner.pendingDamage += 1;
                    recordFeed(hc instanceof MirrorBall ? hc : b, this.owner);
                    this.scalingDedupe.push(targetId);
                }
            }
        }
    }
...
class WrenchBall extends Ball {
    constructor(x, y, vx, vy, theta, dir = 1, hp = 100, radius = 25, color = "#ff9933", mass = radius * radius) {
        ...
        wrench.ballColFns.push((b, reflector, contactPoint) => {
            ...
            let extraSegs = [];
            if (b.segments && b.segments[0]) {
                extraSegs.push(b.segments[0]);
            }
            else if (b instanceof SnakeSegment) {
                if (b.rank == 0) {
                    extraSegs.push(b.owner);
                }
                for (let i = Math.max(0, b.rank - 2); i <= Math.min(b.owner.segments.length - 1, b.rank + 2); i++) {
                    extraSegs.push(b.owner.segments[i]);
                }
            }
            for (let es of extraSegs) {
                avoidCircles.push({ x: es.x, y: es.y, radius: es.radius });
            }
    ...
    getScalingStats() {
        return { turretCount: this.turretCount };
    }

    getInfoEl(stats) {
        return this.propsToList({
            "Turrets": { text: stats.turretCount, grad: { from: 0, to: 10 } },
        });
    }
}
...
class Turret extends CircleBody {
    ...
    static drawState(ctx, state) {
        ctx.strokeStyle = "#333";
        ctx.lineWidth = 2;
        ctx.fillStyle = state.ownerColor;
        ctx.beginPath();
        ctx.arc(state.x, state.y, state.radius - ctx.lineWidth / 2, 0, Math.PI * 2);
        ctx.stroke();
        ctx.fill();
...
class GrimoireBall extends Ball {
    constructor(x, y, vx, vy, theta, dir = 1, hp = 100, radius = 25, color = "#a3a3c6", mass = radius * radius) {
        ...
        grimoire.ballColFns.push((target, reflector) => {
            ...
            if (minion && this.battle.inRectBounds(minion.x, minion.y, minion.radius)) {
                ...
                this.summonCooldown = this.battle.mode == FFA && this.owner ? 50 : this.battle.mode == DUEL && minion.segments ? 25 : 0;
                // this.summonCooldown = 0;
    ...
    createMinion(target, reflector) {
        ...
        if (!this.battle.inRectBounds(minion.x, minion.y, minion.radius)) return null;
        ...
        if (!this.battle.headless) this.battle._flashEvents?.push(minion.id);
    ...
    copyBoosts(target, minion, segmentsNerf = 0) {
        ...
        if (target instanceof MachineGunBall) {
            ...
            minion.dmgCooldown ??= {}; // nor dmgCooldown/segCooldown, needed once this minion acts as a snake-bounce reflector
            minion.segCooldown ??= 0;
    ...
    getScalingStats() {
        return { nextMinionHP: this.nextMinionHP };
    }

    getInfoEl(stats) {
        return this.propsToList({
            "Minion HP": { text: stats.nextMinionHP + this.minionHPGain, grad: { from: this.minionHPGain, to: (this.battle.mode == FFA || this.giga ? 15 : 25) } },
        });
    }
...
class GrowerBall extends Ball {
    ...
    getInfoEl(stats) {
        return this.propsToList({
            "Size Boost": { text: Math.round((stats.scale ** 2 - 1) * 100) + "%", grad: { from: 1, to: 1000 } },
        });
    }
    ...
    handleCollision(b, reflector) {
        ...
        if (!(b instanceof GrowerBall || b instanceof DuplicatorBall || b instanceof SnakeSegment)) {
    ...
    applyGrow(owner) {
        ...
        recordFeed(target, owner);

        const growth =
            target instanceof GrowerBall && !reflector ? 0.15 :
        ...
        owner.growCooldown = growCooldown * (owner.scale ** 2 - prevScale ** 2) / 0.3;
    ...
    getDmgResistance() {
        return /*this.battle.mode == RAID ? this.scale ** 2 * 0.25 + 0.75 :*/ this.scale ** 2 * 0.2 + 0.8;
    }
...
class HammerBall extends Ball {
    constructor(x, y, vx, vy, theta, dir = 1, hp = 100, radius = 25, color = "#c87941", mass = radius * radius) {
        ...
        hammer.ballColFns.push((b, reflector) => {
            this.spinRate += 0.5;
            recordFeed(reflector ?? b, this);
    ...
    getScalingStats() {
        return { spinRate: this.spinRate };
    }

    getInfoEl(stats) {
        return this.propsToList({
            "Acceleration": { text: stats.spinRate.toFixed(1) + "x", grad: { from: 1, to: 10 } },
        });
    }
}
...
class ClubBall extends Ball {
    constructor(x, y, vx, vy, theta, dir = 1, hp = 100, radius = 25, color = "#7b5ea7", mass = radius * radius) {
        ...
        club.ballColFns.push((b, reflector) => {
            ...
            if (sd > b.stunTime) {
                if (!b.isStunned()) {
                    this.stunDur += this.stunIncrease;
                    recordFeed(reflector ?? b, this);
                }
                b._pendingStun = sd;
            }
    ...
    getScalingStats() {
        return { stunDur: this.stunDur };
    }

    getInfoEl(stats) {
        return this.propsToList({
            "Stun Duration": { text: stats.stunDur / 100 + "s", grad: { from: this.stunIncrease / 100, to: 20 * this.stunIncrease / 100 } },
        });
    }

    onLoad() {
        this.stunIncrease = this.battle.mode == FFA ? 60 : this.battle.mode == RAID && !this.giga ? 15 : 15;
...
class MagnetBall extends Ball {
    ...
    handleUpdate(dt) {
        ...
        for (const b of this.battle.balls) {
            if (b.team == this.team || b instanceof SnakeSegment || (b.isStunned() && !b.giga)) continue;

            let bx = b.x, by = b.y;
            if (b instanceof MirrorBall && !b.giga) {
                const att = (b.radius + b.weapons[0].range);
                bx += att * Math.cos(b.weapons[0].theta);
                by += att * Math.sin(b.weapons[0].theta);
            }

            const magnetX = this.x + Math.cos(magnet.theta) * (magnet.range + b.radius);
            const magnetY = this.y + Math.sin(magnet.theta) * (magnet.range + b.radius);
        ...
        targets.forEach(({ b, bx, by, dx, dy, dist }, rank) => {
            ...
            let pull = Math.min(0.25, (b.giga ? 800 : this.battle.mode == FFA || this.giga ? 200 : this.battle.mode == RAID ? 100 : 300) * this.attraction / dist ** 2) * dt;
            const rankWeight = weaponPullDecay ** rank;
            weaponPull -= rankWeight * (b.giga ? 3.28 : this.giga ? 0.82 : 1.64) * Math.sqrt(this.attraction) * Math.sin(this.weapons[0].theta - Math.atan2(by - this.y, bx - this.x)) / (dist + 50);

            const pulled = b.giga ? this : b;
            if (b.giga) pull *= -1;
            const speedBefore = Math.hypot(pulled.vx, pulled.vy);
            pulled.vx -= nx * pull;
            pulled.vy -= ny * pull;
            const speedAfter = Math.hypot(pulled.vx, pulled.vy);
            pulled.knockBoost += 0.5 * (speedAfter * speedAfter - speedBefore * speedBefore);
        });
    ...
    getScalingStats() {
        return { attraction: this.attraction };
    }

    getInfoEl(stats) {
        return this.propsToList({
            "Pull": { text: stats.attraction.toFixed(1) + "x", grad: { from: 1, to: 20 } },
        });
    }
}
...
class SnakeSegment extends Ball {
    constructor(x, y, owner, leader, radius) {
        super(x, y, 0, 0, 1, radius, owner.color, (owner.giga ? 375 : (owner.baseMass ?? owner.mass)) * segRadius / headRadius);
        this.gravity = false;
        this.owner = owner;
        this.team = owner.team;
        this.leader = leader; // the body (head or previous segment) this one is linked to
        this.rank = leader instanceof SnakeSegment ? leader.rank + 1 : 0;
        this.dmgCooldown = {};
        this.dormant = true;
        this.deferredHits = [];
        this.hp = Infinity;
    ...
    damage(dmg, source, srcType) {
        ...
        if (!this.battle.headless) this.battle._flashEvents?.push(this.id);
    }

    onCollision(b) {
        if (!(b instanceof Bullet) && this.dormant) {
            if (!(b instanceof Ball && b.isStunned())) {
                this.handleCollision(b);
                const dx = b.x - this.x, dy = b.y - this.y;
                const dist = Math.hypot(dx, dy) || 1;
                const nx = dx / dist, ny = dy / dist;
                reflectOffPinnedBody(b, nx, ny, 0, 0);
            }
            return;
        }
        super.onCollision(b);
    ...
    shouldBounce(other) { return this.dormant ? false : !(other instanceof Ball && other.isStunned()); }

    getZIndex() {
        return super.getZIndex() - (this.rank + 1);
    }
    ...
    handleUpdate(dt) {
        ...
        if (this.dormant) {
            ...
            if (dist >= this.leader.radius + this.radius - (this.giga ? 7.5 : snakeSegOverlap)) {
                this.dormant = false;
                this.gravity = true;
                ...
                this.owner.extraEnergy += surplus * (this.battle.mode == DUEL ? 0.5 : 1);
            }
            return;
        }
...
class SnakeBall extends Ball {
    constructor(x, y, vx, vy, hp = 100, radius = headRadius, color = "#e0d030", mass = radius * radius) {
        super(x, y, vx, vy, hp, radius, color, mass);
        this.segCooldown = 0;
        this.dmgCooldown = {};
        this.segments = [];
    ...
    handleCollision(b, reflector) {
        ...
        owner.dmgCooldown[b.id] = this.battle.mode == FFA ? 10 : 3;
        b.damage(1, owner);
        ...
        if (owner.segCooldown <= EPS) {
            ...
            owner.segCooldown = this.battle.mode == FFA ? 20 : this.giga ? 3 : 10;
            ...
            const newSegRadius = owner.giga ? 15 : segRadius * ownerScale;
    ...
    getScalingStats() {
        return { segCount: this.segments.length };
    }

    getInfoEl(stats) {
        return this.propsToList({
            "Segments": { text: stats.segCount, grad: { from: 0, to: 20 } },
        });
    }
}

// Vampire: Drains life but passively loses HP
class VampireBall extends Ball {
    ...
    getScalingStats() {
        return { lifesteal: this.lifesteal };
    }

    getInfoEl(stats) {
        return this.propsToList({
            "Lifesteal": { text: stats.lifesteal, grad: { from: 1, to: 10 } },
        });
    }
...
class CloverBall extends Ball {
    constructor(x, y, vx, vy, theta, dir = 1, hp = 100, radius = 25, color = "#3fae4a", mass = radius * radius) {
        ...
        clover.ballColFns.push((b, reflector) => {
            ...
            if (reflector) {
                ...
                this.battle._pendingCloverCheats.push({ clover: reflector, bounced: null, reflected: this });
            }
        });
    ...
    onLoad() {
        if (this.battle.mode == FFA) this.weapons[0].iframes = 40;
    }
    ...
    static snapshotBodies(bodies, flashedIds = null) {
        const flashed = flashedIds && new Set(flashedIds);
        return bodies.map(b => {
            if (b instanceof Ball) {
                return {
                    isBall: true,
    ...
    async scoreCandidate(deltaRad, lookahead, bounced, yieldState) {
        ...
        fork.evil = true;
        if (debugTiming) {
            console.log(`[t=${this.battle.t}] Clover scoreCandidate: evilFork() took ${(performance.now() - forkT0).toFixed(2)}ms delta=${deltaRad.toFixed(4)} lookahead=${lookahead} bodies=${this.battle.bodies.length}`);
        }
        ...
        // fork is headless, so nothing else drains _flashEvents -- read+clear it
        // ourselves each tick to know which balls took a real damage event (as
        // opposed to e.g. VampireBall's passive bleed(), which mutates hp directly).
        fork._flashEvents = [];
        const path = [CloverBall.snapshotBodies(fork.bodies, fork._flashEvents)];
        fork._flashEvents = [];
        ...
        for (let i = 0; i < lookahead; i++) {
            ...
            await fork.update();
            fork.updateTimeScale();
            path.push(CloverBall.snapshotBodies(fork.bodies, fork._flashEvents));
            fork._flashEvents = [];
            rewards.push(rewardAt());
            timeScales.push(fork.timeScale);
            if (fSelf._bounceCount >= 20) break;

            if (yieldState) {
                yieldState.ticks++;
                if (yieldState.ticks >= this.battle.getYieldTicks()) {
                    yieldState.ticks = 0;
                    await cheatYield(this.battle);
                }
            }
        }
    ...
    async cheat(bounced = null) {
        ...
        if (bestDelta !== 0 && !this.battle.headless) {
            ...
            const relRewards = noCheatRewards.map((r, i) => r - (chosen.rewards[i] ?? chosen.rewards[chosen.rewards.length - 1]));
            this.battle.cloverGhosts = { startTick: this.battle.t, paths: [/*mirrorPath,*/ noCheatPath], rewards: [relRewards], timeScales: noCheatTimeScales };
        }
    ...
    getScalingStats() {
        return { foresight: this.foresight };
    }

    getInfoEl(stats) {
        return this.propsToList({
            "Cheat Duration": { text: (stats.foresight / 100).toFixed(1) + "s", grad: { from: 0, to: 2.5 } },
        });
    }
}
...
class DamageIndicator {
    constructor(ballId, dmg, color, comboGroup, isHeal, hpBeforeCeil, hpAfterCeil) {
        this.ballId = ballId;
        this.comboGroup = comboGroup;
        this.isHeal = isHeal;
        this.color = isHeal ? "#1a9e1a" : mixWithBlack(color, 50);
        this.hpBeforeCeil = hpBeforeCeil;
        this.setDmg(dmg, hpAfterCeil);
        this.x = 0;
        this.y = 0;
        this.yOffset = 0;
        this.life = 1;
    ...
    setDmg(dmg, hpAfterCeil) {
        this.dmg = dmg;
        this.displayedDmg = this.hpBeforeCeil == Infinity ? 0
            : this.isHeal ? hpAfterCeil - this.hpBeforeCeil
                : this.hpBeforeCeil - hpAfterCeil;
        this.fontSize = 20 + 2 * (this.displayedDmg == 0 ? 0 : Math.log(this.displayedDmg));
    }

    reset(radius) {
        this.yOffset = 5 + radius + this.fontSize / 2;
        this.life = 1;
    }

    update(realDt = 16.6667, ballPos = null) {
        const vy = Math.min(1, (1 - this.life) * 2);
        if (this.life > indicatorComboThresh) {
            this.yOffset += vy;
            if (ballPos) { this.x = ballPos.x; this.y = ballPos.y - this.yOffset; }
            else this.y -= vy;
        }
        else {
            this.y -= vy;
        }
        this.life -= realDt / 600;
    }
    ...
    draw(ctx) {
        ...
        ctx.fillText((this.isHeal ? "+" : "") + this.displayedDmg, this.x, this.y);
        ctx.globalAlpha = 1;
    }
}
